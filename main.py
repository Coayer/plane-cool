...
def calculate_heading_to_aircraft(user_lat, user_lon, aircraft_lat, aircraft_lon):
    ...
    x = math.cos(user_lat_rad) * math.sin(aircraft_lat_rad) - math.sin(user_lat_rad) * math.cos(aircraft_lat_rad) * math.cos(dlon)
    ...
    try:
        ...
        for flight in flights:
            ...
            if flight_data is None:
                aircraft_data.append((float('inf'), None, f"Error processing flight {flight}"))
                continue
...
def get_aircraft():
    try:
        ...
        for dist, flight_data, heading_to_aircraft in aircraft_data:
            if flight_data is None:
                output_strings.append(heading_to_aircraft)  # This contains the error message
            else:
                dist_km = dist / 1000
...
def get_closest_aircraft():
    try:
        ...
        valid_aircraft = [(dist, flight_data) for dist, flight_data, _ in aircraft_data if flight_data is not None]

        if not valid_aircraft:
            return "No aircraft found within 10km of your location.", 200, {'Content-Type': 'text/plain'}
        ...
        sentence = f"{closest_flight['airline']} {closest_flight['aircraft_type']} departing from {closest_flight['source']}."
